...
var unprefix = R.pipe(R.split('R.'), R.prop(1));
var trimCode = R.pipe(R.split(/\n/), R.map(R.trim), R.join('\n'));
var prettifySig = R.pipe(R.replace(/\.{3}/g, '\u2026'), R.replace(/->/g, '\u2192'));
var prettifyCode = R.pipe(R.join('\n'), trimCode, R.lPartial(hljs.highlight, 'javascript'), R.prop('value'));
...
var nonPrivateAccess = R.pipe(R.prop('access'), R.not(R.eq('private')));
...
function publish(data, opts) {
  ...
  var fullData = data().order('name, version, since').filter({kind: 'function'}).get().filter(nonPrivateAccess).map(simplifyData);
  ...
  var indexHtml = indexTmpl({version: pkg.version});
